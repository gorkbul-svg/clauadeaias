...
async def debug_zanger():
    ...
    def kontrol():
        ...
        # Erişim testleri
        test1 = test2 = test3 = test4 = "hata"
        try:
            test1 = str(df["GARAN.IS"].shape)
        except Exception as e:
            test1 = f"hata: {e}"
        try:
            test2 = str(df["GARAN.IS"]["Close"].iloc[-1])
        except Exception as e:
            test2 = f"hata: {e}"
        try:
            test3 = str(df.loc[:, "GARAN.IS"].shape)
        except Exception as e:
            test3 = f"hata: {e}"
        try:
            test4 = str(df["GARAN.IS", "Close"].iloc[-1])
        except Exception as e:
            test4 = f"hata: {e}"

        return {
            "shape": str(df.shape),
            "level0_sample": str(df.columns.get_level_values(0)[:4].tolist()),
            "level1_sample": str(df.columns.get_level_values(1)[:4].tolist()),
            "test_bracket": test1,
            "test_close": test2,
            "test_loc": test3,
            "test_tuple": test4,
