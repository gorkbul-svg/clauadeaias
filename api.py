        ...
        for ticker in hedef_liste:
            try:
                ...
                if len(hedef_liste)==1:
                    ...
                    try:
                        df = df_bulk.loc[:, (slice(None), sembol)]
...
async def debug_zanger():
    ...
    def kontrol():
        semboller = ["GARAN.IS", "AKBNK.IS", "THYAO.IS"]
        df = yf.download(semboller, period="5d", interval="1d", progress=False, auto_adjust=True, group_by="ticker")
        return {
            "columns": str(df.columns.tolist()[:10]),
            "shape": str(df.shape),
            "col_type": str(type(df.columns)),
            "has_levels": hasattr(df.columns, 'levels'),
            "nlevels": df.columns.nlevels if hasattr(df.columns, 'nlevels') else 0,
            "sample_cols": str(df.columns[:6].tolist()) if len(df.columns) > 0 else "empty",
        }
...
async def zanger_tarayici(batch: int = 0):
    ...
    def tara():
        ...
        # BIST100 getirisi
        try:
            df_xu = yf.download("XU100.IS", period="3mo", interval="1d", progress=False, auto_adjust=True)
            bist_getiri = float(
        ...
        for ticker in hedef_liste:
            try:
                ...
                if len(hedef_liste) == 1:
                    ...
                    # yfinance multi-ticker: (field, ticker) şeklinde MultiIndex
                    cols = df_bulk.columns
                    if hasattr(cols, 'levels'):
                        # MultiIndex — tüm field'ları al
                        try:
                            closes = df_bulk["Close"][sembol] if sembol in df_bulk["Close"].columns else None
                            if closes is None:
                                continue
                            df = df_bulk.loc[:, (slice(None), sembol)]
                            df.columns = df.columns.droplevel(1)
                ...
                k4 = baz_genislik <= 8.0
                baz_tipi = "Sıkışma" if baz_genislik <= 3 else "Dar Baz" if baz_genislik <= 6 else "Geniş Baz"
                ...
                guc = "🔥 MÜKEMMEL" if skor==5 else "⭐ GÜÇLÜ" if skor==4 else "✅ İYİ" if skor==3 else "👀 TAKİPTE"
                ...
                sonuclar.append({
                    "ticker": ticker, "fiyat": round(gun_fiyat,2), "degisim": fiyat_degisim,
                    "skor": skor, "guc": guc, "kriterler": kriterler,
                    "zirveye_uzaklik": round(zirveye_uzaklik,1), "zirve_52h": round(zirve,2),
                    "hacim_oran": round(hacim_oran,1), "rs_skor": rs_skor, "rs_fark": round(rs_fark,1),
                    "baz_tipi": baz_tipi, "baz_genislik": round(baz_genislik,1),
                    "dun_yuksek": round(float(yuksek[-2]),2), "atr": atr,
                    "hisse_getiri_3ay": round(hisse_getiri,1),
                })
            except Exception as e:
                print(f"[ZANGER] {ticker}: {e}")
        ...
        return {
            "hisseler":    sonuclar,
            "toplam":      len(sonuclar),
            "taranan":     len(hedef_liste),
            "bist_getiri": round(bist_getiri, 1),
            "batch":       batch,
            "son_batch":   bitis >= len(BIST100),
        }
