...
_jobs: dict = {}  # job_id → {durum, sonuc, hata, baslangic}

def job_baslat(job_id: str, fn, *args):
    """Fonksiyonu arka planda çalıştır, sonucu job store'a yaz."""
    _jobs[job_id] = {"durum": "calisıyor", "sonuc": None, "hata": None, "baslangic": time.time()}
    def _calistir():
        try:
            sonuc = fn(*args)
            _jobs[job_id]["sonuc"] = sonuc
...
async def firsat_tarayici():
    ...
    def tara():
        ...
        return {"firsatlar": sonuclar, "toplam": len(sonuclar), "taranan": len(BIST100)}

    job_id = str(uuid.uuid4())
    job_baslat(job_id, tara)
    return {"job_id": job_id, "durum": "baslatildi"}
