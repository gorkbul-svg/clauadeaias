...
async def firsat_tarayici(batch: int = 0):
    ...
    def tara():
        ...
        for ticker in hedef_liste:
            try:
                sembol = f"{ticker}.IS"
                df = df_bulk if len(hedef_liste)==1 else (df_bulk[sembol] if sembol in df_bulk.columns.get_level_values(1) else None)
...
async def para_akisi(batch: int = 0):
    ...
    def tara():
        ...
        for ticker in hedef_liste:
            try:
                sembol = f"{ticker}.IS"
                df_s = df_bulk  if len(hedef_liste)==1 else (df_bulk[sembol]   if sembol in df_bulk.columns.get_level_values(1)   else None)
                df_g = df_bulk_g if len(hedef_liste)==1 else (df_bulk_g[sembol] if sembol in df_bulk_g.columns.get_level_values(1) else None)
...
async def zanger_tarayici(batch: int = 0):
    ...
    def tara():
        ...
        for ticker in hedef_liste:
            try:
                sembol = f"{ticker}.IS"
                # Tek hisse vs çoklu hisse DataFrame yapısı farklı
                if len(hedef_liste) == 1:
                    df = df_bulk
                else:
                    df = df_bulk[sembol] if sembol in df_bulk.columns.get_level_values(1) else df_bulk.get(sembol)
