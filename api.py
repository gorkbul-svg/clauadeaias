...
# ── Fırsat Tarayıcı ───────────────────────────────────────
BIST100 = [
    "GARAN","AKBNK","YKBNK","ISCTR","HALKB","VAKBN","TSKB","ALBRK",
    "THYAO","PGSUS","TAVHL","CLEBI",
    "EREGL","KRDMD","CEMTS","TRKCM","OTKAR",
    "TUPRS","AKENR","AKSEN","ZOREN","ODAS",
    "ASELS","NETAS","LOGO","DOAS","INDES","ARMDA",
    "FROTO","TOASO","MGROS","BIMAS","SOKM","MAVI","LCWGN",
    "KCHOL","SAHOL","SISE","DOHOL","TKFEN","AGHOL",
    "ISGYO","EKGYO","TRGYO","PETKM","GUBRF","ENKAI",
    "KOZAL","KORDS","CIMSA","AKCNS","BRISA","VESBE",
    "SODA","ECZYT","ULKER","TTKOM","TCELL","ARCLK",
]

@app.get("/bist/firsat-tarayici")
async def firsat_tarayici():
    """
    Tüm BIST'i tarar, dip + kalkış hazırlığındaki hisseleri bulur.
    6 sinyal: RSI dip dönüş, Hacim artışı, Destek yaklaşımı,
              MACD pozitif kesişim, Bollinger alt bant, Yabancı yoğunlaşması
    """
    import yfinance as yf
    import numpy as np
    from fastapi.concurrency import run_in_threadpool

    def hisse_tara(ticker: str) -> dict | None:
        try:
            stock = yf.Ticker(f"{ticker}.IS")
    ...
    def tara():
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(hisse_tara, t): t for t in BIST100}
    ...
    firsatlar = await run_in_threadpool(tara)
    return {
        "firsatlar": firsatlar,
        "toplam":    len(firsatlar),
        "taranan":   len(BIST100),
    }
...
async def para_akisi():
    """
    Tüm BIST'i tarar. Her hisse için:
    - MFI (Money Flow Index): 14 günlük para akışı gücü
    - OBV (On-Balance Volume): birikimli hacim yönü
    - Hacim analizi: son gün hacmi ortalamayla karşılaştır
    - Para Akış Skoru: 3 indikatörün ağırlıklı ortalaması
    """
    import yfinance as yf
    import numpy as np
    from fastapi.concurrency import run_in_threadpool

    def hisse_para_akisi(ticker: str) -> dict | None:
        try:
            stock = yf.Ticker(f"{ticker}.IS")

            # ── 1 Saatlik veri (son 60 gün) ───────────────
            df_saat = stock.history(period="60d", interval="1h")
            # ── Günlük veri (OBV trendi için) ─────────────
    ...
    def tara():
        ...
        sonuclar = []
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(hisse_para_akisi, t): t for t in BIST100}
            for future in as_completed(futures):
                try:
                    r = future.result(timeout=15)
                    if r:
    ...
    hisseler = await run_in_threadpool(tara)

    # Özet istatistikler
    giren  = [h for h in hisseler if h["yon"] == "giris"]
    cikan  = [h for h in hisseler if h["yon"] == "cikis"]
    notr   = [h for h in hisseler if h["yon"] == "notr"]
    ...
    return {
        ...
        "ozet": {
            ...
            "cikan":   len(cikan),
            "notr":    len(notr),
            "ortalama_skor": round(sum(h["para_skoru"] for h in hisseler) / len(hisseler), 1) if hisseler else 0,
        }
    }

# ── Zanger Tarayıcı ───────────────────────────────────────
@app.get("/bist/zanger")
async def zanger_tarayici():
    """
    Dan Zanger yaklaşımıyla BIST tarar.
    5 kriter:
    1. 52 haftalık zirveye yakınlık (%5 içinde)
    2. Hacim patlaması (2x+ ortalama)
    3. Relative Strength (BIST100'e göre 3 aylık outperform)
    4. Baz oluşumu (son 3-10 gün düşük volatilite)
    5. Kırılım (dünkü high kırıldı mı?)
    """
    import yfinance as yf
    import numpy as np
    from fastapi.concurrency import run_in_threadpool
    ...
    def tara():
        # Önce BIST100 getirisi hesapla (benchmark)
        try:
            xu100 = yf.Ticker("XU100.IS")
            df_xu = xu100.history(period="3mo", interval="1d")
            if not df_xu.empty and len(df_xu) >= 2:
                bist_getiri = float(
                    (df_xu["Close"].iloc[-1] - df_xu["Close"].iloc[-63]) /
                    df_xu["Close"].iloc[-63] * 100
                ) if len(df_xu) >= 63 else 0.0
            else:
                bist_getiri = 0.0
        except:
            bist_getiri = 0.0
        ...
        sonuclar = []
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = {executor.submit(zanger_hesapla, t, bist_getiri): t for t in BIST100}
            for future in as_completed(futures):
                try:
                    r = future.result(timeout=15)
                    if r:
        ...
        return sonuclar, bist_getiri

    sonuclar, bist_getiri = await run_in_threadpool(tara)

    return {
        "hisseler":    sonuclar,
        "toplam":      len(sonuclar),
        "taranan":     len(BIST100),
        "bist_getiri": round(bist_getiri, 1),
        "kriterler_aciklama": {
            "zirve_yakini":    "52 haftalık zirveye %5 içinde",
            "hacim_patlamasi": "Bugünkü hacim 50 günlük ortalamanın 2x+",
            "relative_guc":    "Son 3 ayda BIST100'ü geçiyor",
            "baz_olusumu":     "Son 10 günde %8 içinde bant",
            "kirilim":         "Dünkü zirveyi hacimle kırdı",
        }
    }
