...
async def firsat_tarayici(batch: int = 0):
    ...
    def tara():
        ...
        with ThreadPoolExecutor(max_workers=15) as executor:
            futures = {executor.submit(hisse_tara, t): t for t in hedef_liste}
        ...
        return {"firsatlar": sonuclar, "toplam": len(sonuclar), "taranan": len(hedef_liste), "son_batch": (batch+1)*20 >= len(BIST100)}
...
async def para_akisi(batch: int = 0):
    ...
    def tara():
        ...
        sonuclar = []
        with ThreadPoolExecutor(max_workers=15) as executor:
            futures = {executor.submit(hisse_para_akisi, t): t for t in hedef_liste}
            for future in as_completed(futures):
                try:
                    r = future.result(timeout=15)
                    if r:
...
async def zanger_tarayici(batch: int = 0):
    ...
    def tara():
        ...
        sonuclar = []
        with ThreadPoolExecutor(max_workers=15) as executor:
            futures = {executor.submit(zanger_hesapla, t, bist_getiri): t for t in hedef_liste}
            for future in as_completed(futures):
                try:
                    r = future.result(timeout=15)
                    if r:
