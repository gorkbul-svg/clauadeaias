...
async def para_akisi():
    ...
    def hisse_para_akisi(ticker: str) -> dict | None:
        try:
            ...
            # 14 günlük MFI
            period = 14
            pos_sum = sum(positive_mf[-period:])
            neg_sum = sum(negative_mf[-period:])
            if neg_sum == 0:
                mfi = 100.0
            else:
            ...
            for i in range(1, len(kapanis)):
                if kapanis[i] > kapanis[i-1]:
                    obv.append(obv[-1] + hacim[i])
                elif kapanis[i] < kapanis[i-1]:
                    obv.append(obv[-1] - hacim[i])
                else:
                    obv.append(obv[-1])
            obv = np.array(obv)
            ...
            hacim_oran    = hacim_bugun / hacim_ort20 if hacim_ort20 > 0 else 1

            # Yön teyidi: hacim artışı + fiyat yönü
            ...
            # ── Para Akış Skoru (ağırlıklı) ──────────────
            # MFI %40 + OBV %35 + Hacim %25
            ...
            # ── 5 günlük trend ────────────────────────────
            # Her gün için basit para akışı yönü
            trend = []
            for i in range(-5, 0):
                tp_gun = (yuksek[i] + dusuk[i] + kapanis[i]) / 3
                tp_onc = (yuksek[i-1] + dusuk[i-1] + kapanis[i-1]) / 3
                mf_gun = tp_gun * hacim[i]
                trend.append({
                    "yon":   "↑" if tp_gun > tp_onc else "↓",
                    "guc":   round(mf_gun / 1e6, 1),  # Milyon TL
                })
            ...
            return {
                ...
                "trend":        trend,
                "hacim_tl":     round(hacim_bugun * gun_fiyat / 1e6, 1),  # Milyon TL
            }
        except Exception as e:
